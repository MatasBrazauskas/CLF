...
static std::vector<GroupInfo> mostRequestPerData(std::unordered_map<std::string, ReqAndBytesCnt>& map, const std::size_t n);
...
std::vector<GroupInfo> mostRequestPerData(std::unordered_map<std::string, ReqAndBytesCnt>& map, const std::size_t n) {
    const auto sortingPredicate = [](const GroupInfo& a, const GroupInfo& b) {
        return a.requestCount > b.requestCount;
    };

    const auto transformFunction = [](const std::pair<std::string, ReqAndBytesCnt>& pair) {
